...
- Change `"basePrice"` or `"sizeUpcharge"` to update pricing (upcharge is added for the larger size).
...
## 5. Changing flavours, sizes, or the logo text

Open `data/config.json`:

```json
{
  "flavors": ["Strawberry", "Kiwi", "Coconut", "Lavender", "Rose", "Vanilla", "Green Tea", "Citrus"],
  "sizes": ["100g", "250g"],
  "applications": ["Hand", "Foot"]
}
```

Add, remove, or rename any of these lists and every product page updates automatically — no other file needs to change.
